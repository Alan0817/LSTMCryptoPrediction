...
def build_document_retriever(
    ...
    index_dir="data/financial_documents/semantic_index",
    reranker=None,
):
    backend = backend or os.getenv("RETRIEVAL_BACKEND") or DEFAULT_BACKEND
    if backend not in VALID_BACKENDS:
        raise ValueError("Unsupported retrieval backend: {!r}.".format(backend))

    chunks = list(chunks) if chunks is not None else CorpusStorage().load_chunks()
    if not chunks:
        raise RuntimeError("Document corpus is unavailable or empty.")
    if backend == "bm25":
    ...
    return HybridRetriever(
        ...
        reranker=reranker or CrossEncoderReranker(device="cpu"),
        candidate_k=20,
        rrf_k=60,
    )
